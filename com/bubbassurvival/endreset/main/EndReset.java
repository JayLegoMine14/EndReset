...
public class EndReset extends JavaPlugin implements Listener, Runnable {
    ...
    public void onEnable(){
        ...
        try{
            ...
            this.getServer()
                    .getConsoleSender()
                    .sendMessage(ChatColor.RED + "Error loading config.yml!" + " Unparsable value in config! Caused by: "
                            + e.getMessage());
        }
                ...
                if(sfv < 6){
                    ...
                    if(sfv > 2){
                        ...
                        for(final Object e3: ((HashMap) sa[6]).entrySet()){
                            this.forceReset.put(((Map.Entry<String, V10World>) e3).getKey(), ((Map.Entry<String, V10World>) e3)
                                    .getValue());
                        }
                        for(final Object e4: ((HashMap) sa[7]).entrySet()){
                            this.dragonAmount.put(((Map.Entry<String, Short>) e4).getKey(), ((Map.Entry<String, Short>) e4)
                                    .getValue());
                        }
                    }
                    this.save = save;
                    ...
                    for(final Object e5: ((HashMap) in.readObject()).entrySet()){
                        this.v10chunks
                                .put(((Map.Entry<String, HashMap<String, Long>>) e5).getKey(), ((Map.Entry<String, HashMap<String, Long>>) e5)
                                        .getValue());
                    }
                    ...
                    for(final Object e7: ((HashMap) in.readObject()).entrySet()){
                        this.forceReset.put(((Map.Entry<String, V10World>) e7).getKey(), ((Map.Entry<String, V10World>) e7)
                                .getValue());
                    }
                    for(final Object e8: ((HashMap) in.readObject()).entrySet()){
                        this.dragonAmount.put(((Map.Entry<String, Short>) e8).getKey(), ((Map.Entry<String, Short>) e8)
                                .getValue());
                    }
                    for(final Object e6: ((HashMap) in.readObject()).entrySet()){
        ...
        // bs.scheduleSyncRepeatingTask((Plugin)this, new HealthThread(), 1L,
        // 50L);
     ...
     * 
     * @Override public void run() { for(World w : s.getWorlds()){
     * if(w.getEnvironment() == World.Environment.THE_END){ for(Entity ae :
     * w.getEntities()){ if(ae.getType() == EntityType.ENDER_DRAGON){
     * LivingEntity e = (LivingEntity) ae; if(e.getHealth() + 1 <
     * e.getMaxHealth() && e.getHealth() != 0.0) e.setHealth(e.getHealth() + 1);
     * } } } } } }
     */
